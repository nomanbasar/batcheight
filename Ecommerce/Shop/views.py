...
def lehenga(request, data = None):
 if data == None:
   ...
   lehengas = Product.objects.filter(category='L').filter(discounted_price__lt = 30000)
 
 elif data == 'above':
   lehengas = Product.objects.filter(category='L').filter(discounted_price__gt=30000)
...
def plus_cart(request):
  if request.method == 'GET':
    ...
    for p in cart_product:
      ...
      amount += tempamount
      totalamount = amount + shipping_amount
    product_total_cost = c.quantity * c.product.discounted_price
